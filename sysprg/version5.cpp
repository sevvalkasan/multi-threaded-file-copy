...
void copy_directory_recursive(const fs::path& source_dir, const fs::path& destination_dir, std::atomic<int>& files_copied) {
...
void copy_files(const fs::path& source_dir, const fs::path& destination_dir) {
    if (fs::exists(source_dir) && fs::is_directory(source_dir)) {
        ...
        // Dosyaları kopyalamak için her bir iş parçacığı oluşturulur
        for (unsigned int i = 0; i < std::thread::hardware_concurrency(); ++i) {
            threads.emplace_back(copy_directory_recursive, std::cref(source_dir), std::cref(destination_dir), std::ref(files_copied));
        }

        // İş parçacıklarının tamamlanması beklenir
